...
public class GetByLanguageCodeQueryHandler(IAboutRepository aboutRepository)
    : IQueryHandler<GetByLanguageCodeQuery, AboutDto>
{
    public async Task<Result<AboutDto>> Handle(GetByLanguageCodeQuery request, CancellationToken cancellationToken)
    {
        if (!Enum.TryParse<LanguageCode>(request.LanguageCode, true, out var languageCode))
